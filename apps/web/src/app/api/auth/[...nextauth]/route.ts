...
export const GET = async (
  ...
  const isOutsourcedSignIn = req.nextUrl.searchParams.get("redirect");
  const isOutsourcedCallback = cookies().get(REDIRECT_COOKIE_NAME);

  if (nextauthAction === "signin" && !!isOutsourcedSignIn) {
    // set a cookie we can read in the callback
    // to know to send the user back to expo
    cookies().set({
      name: REDIRECT_COOKIE_NAME,
      value: isOutsourcedSignIn,
      maxAge: 60 * 10, // 10 min
      path: "/",
    });
  }

  if (nextauthAction === "callback" && !!isOutsourcedCallback) {
    ...
    const url = new URL(isOutsourcedCallback.value);
